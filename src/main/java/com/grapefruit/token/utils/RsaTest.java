...
import static com.grapefruit.token.utils.RSAUtils.getPrivateKey;
import static com.grapefruit.token.utils.RSAUtils.getPublicKey;
...
public class RsaTest {
    ...
    private static void signAndCheck(Map<String, String> map) throws NoSuchAlgorithmException, InvalidKeySpecException, SignatureException, InvalidKeyException {
        ...
        String sign = RSAUtils.sign(getPrivateKey(privateKey), plainText);

        String sign2 = RSAUtils.sign(getPrivateKey(privateKey), "plainText");

        boolean verify = RSAUtils.verify(plainText, getPublicKey(publicKey), sign2);
